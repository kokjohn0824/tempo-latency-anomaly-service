...
func TraceLookup(client *tempo.Client, checker *service.Check) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Add isAnomaly to each trace when checker is available.
		// Note: anomaly evaluation uses each trace's root service/name to match baseline keys.
		if checker != nil {
			annotated, err := checker.AnnotateTraces(r.Context(), traces)
			if err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
...
// TraceLookupAnomalies godoc
// @Summary Look up anomalous traces
// @Description Same as /v1/traces, but returns only traces whose computed isAnomaly=true
// @Tags Traces
// @Accept json
// @Produce json
// @Param service query string true "Service name" example("api-gateway")
...
// @Router /v1/traces/anomalies [get]
func TraceLookupAnomalies(client *tempo.Client, checker *service.Check) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if client == nil {
			http.Error(w, "tempo client not available", http.StatusServiceUnavailable)
			return
		...
		filtered := make([]domain.TraceEvent, 0, len(annotated))
